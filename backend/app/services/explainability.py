class RuleBasedExplainabilityEngine:
    ...
    def generate_explanations(transaction_data: dict, risk_score: float, ml_probability: float) -> list[str]:
        ...
        if amount > avg_spend * 2 and avg_spend > 0:
            explanations.append(f"Transaction amount (${amount}) is significantly higher than customer's average (${avg_spend}).")
            
        # Rule 2: High-risk merchant
        if merchant_risk > 5.0:
            explanations.append(f"Merchant has a high historical fraud rate ({merchant_risk}%).")
            
        # Rule 3: Time of day
        if hour < 5 or hour > 23:
            explanations.append("Transaction occurred at an unusual time (late night/early morning).")
            
        # Rule 4: Device/Location Mismatches (mock logic for demo)
        if transaction_data.get('is_new_device', False):
            explanations.append("Payment initiated from a new or unrecognized device.")
            
        if transaction_data.get('is_new_location', False):
            explanations.append("Transaction originated from an unusual geographic location.")

        # ML Base contribution
        if ml_probability > 0.8:
            explanations.append("Machine Learning model identified high-confidence fraud patterns.")
            
        if not explanations:
            explanations.append("Standard transaction behavior detected. No major risk factors.")
