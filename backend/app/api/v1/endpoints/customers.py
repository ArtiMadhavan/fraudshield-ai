...
def get_customer_360(customer_id: str, db: Session = Depends(get_db)):
    ...
    if not customer:
        # For seamless demo flow, try finding by ID or return a random customer if using simulated customer_id
    ...
    # Get device and location history
    try:
        device_history = json.loads(customer.device_history) if customer.device_history else []
    except:
        device_history = []
        
    try:
        location_history = json.loads(customer.location_history) if customer.location_history else []
    except:
        location_history = []
    ...
    summary = f"Customer {customer.name} (KYC: {customer.kyc_status}) has a risk score of {customer.risk_score:.1f}/100 and a credit score of {customer.credit_score}. They have a wallet balance of ${customer.wallet_balance:.2f}."
